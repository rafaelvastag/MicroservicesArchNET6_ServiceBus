...
namespace OrderAPI.Messaging
{
    public class AzureServiceBusConsumer : IAzureServiceBusConsumer
    {
        ...
        private ServiceBusProcessor checkoutProcessorQueue;
        ...
        public AzureServiceBusConsumer(OrderRepository orderRepository, IConfiguration configuration, IMessageBus messageBus)
        {
            ...
            orderPaymentProcessQueue = _configuration.GetValue<string>("OrderPaymentProcessQueue");
            orderPaymentUpdateResultTopic = _configuration.GetValue<string>("OrderPaymentUpdateResultTopic");

            var client = new ServiceBusClient(serviceBusConnectionString);
            checkoutProcessor = client.CreateProcessor(checkoutMessageTopic, subscriptionCheckout);
            checkoutProcessorQueue = client.CreateProcessor(checkoutMessageQueue);
        ...
        public async Task Start()
        {
            ...
            checkoutProcessorQueue.ProcessMessageAsync += OnCheckOutMessageReceivedQueue;
            checkoutProcessorQueue.ProcessErrorAsync += ErrorHander;

            await checkoutProcessorQueue.StartProcessingAsync();
        }
        ...
        public async Task Stop()
        {
            ...
            await checkoutProcessorQueue.StopProcessingAsync();
            await checkoutProcessorQueue.DisposeAsync();
        }
        ...
        private async Task OnCheckOutMessageReceivedQueue(ProcessMessageEventArgs args)
        {
            var message = args.Message;
            var body = Encoding.UTF8.GetString(message.Body);
            ...
            try
            {
                await _messageBus.PublishMessage(paymentRequestMessage, orderPaymentProcessQueue);
                await args.CompleteMessageAsync(args.Message);
            }
            catch (Exception)
            {
